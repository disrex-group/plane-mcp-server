...
### Fixed
- **Issue Comments**: Fixed comment creation and update functionality
  - Changed API payload to use `comment_html` field instead of `comment`
  - Added automatic HTML formatting for plain text comments
  - Added support for `access` parameter (INTERNAL/EXTERNAL) for comment visibility
  - All comment operations (create, read, update, delete) now work correctly with Plane API
